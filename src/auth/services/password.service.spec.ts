...
describe('PasswordService', () => {
  ...
  describe('hashPassword', () => {
    ...
    it('should throw error for empty password', async () => {
      await expect(service.hashPassword('')).rejects.toThrow(InternalServerErrorException);
    });

    it('should throw error for null password', async () => {
      await expect(service.hashPassword(null as any)).rejects.toThrow(InternalServerErrorException);
    });

    it('should throw error for undefined password', async () => {
      await expect(service.hashPassword(undefined as any)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('validatePassword', () => {
    ...
    it('should handle invalid hash format gracefully', async () => {
      const isValid = await service.validatePassword('password123', 'invalidHashFormat');
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('Security Tests', () => {
    ...
    it('should handle special characters in passwords', async () => {
      ...
      const isValid = await service.validatePassword(specialPassword, hashedPassword);
    ...
    it('should handle unicode characters in passwords', async () => {
      ...
      const isValid = await service.validatePassword(unicodePassword, hashedPassword);
